...
export const IdeHeader = ({ currentView, onBackToDashboard, selectedProject }) => {
    ...
    return (
        <header className={`w-full h-[8%] flex flex-row pr-4 pl-4 justify-between items-center ${
            theme === 'dark' ? 'bg-zinc-900 border-b border-zinc-800' : 'bg-white border-b border-gray-200'
        }`}>
            <div className="flex flex-row items-center gap-3 font-bold">
                <span className="w-10 h-10 bg-orange-500 rounded flex items-center justify-center text-lg">
                    {'</>'}
                </span>
